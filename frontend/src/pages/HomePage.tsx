...
export function HomePage() {
  ...
  return (
    <div style={{ backgroundColor: headerBgColor, minHeight: '100vh', width: '100%', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      <div style={{ position: 'absolute', top: '1rem', right: '12px', zIndex: 100 }}><DarkModeToggle /></div>
      <SEO title="" description="Interactive bus route maps for Portland Public Schools." />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', width: '100%', padding: '2rem', boxSizing: 'border-box' }}>
        <div style={{ width: '100%', maxWidth: '380px' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            {selectedAddress ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.75rem 0.75rem 0.75rem 1rem', backgroundColor: 'var(--bg-primary)', borderRadius: '12px', border: '1px solid var(--border-color)', height: '46px', boxSizing: 'border-box' }}>
                <i className="fas fa-house" style={{ color: 'var(--text-primary)', fontSize: '14px' }}></i>
                <div style={{ flex: 1, fontSize: '14px', color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{selectedAddress.address}</div>
              ...
              <div style={{ position: 'relative' }}>
                <input ref={addressInputRef} type="text" value={addressQuery} onChange={(e) => setAddressQuery(e.target.value)} onKeyDown={handleAddressKeyDown} onFocus={() => { if (addressSuggestions.length > 0) setShowAddressSuggestions(true); }} placeholder="Enter your address..." style={{ width: '100%', padding: '0.75rem 0.75rem 0.75rem 1rem', border: '1px solid var(--border-color)', borderRadius: '12px', fontSize: '14px', backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)', boxSizing: 'border-box', height: '46px', outline: 'none' }} />
                ...
                {showAddressSuggestions && addressSuggestions.length > 0 && (
                  <div ref={addressSuggestionsRef} style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '4px', backgroundColor: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: '12px', boxShadow: '0 2px 8px var(--shadow-hover)', maxHeight: '200px', overflowY: 'auto', zIndex: 1000 }}>
                    {addressSuggestions.map((suggestion, index) => (
                      <div key={index} onClick={() => handleSelectAddress(suggestion)} onMouseEnter={() => setHighlightedAddressIndex(index)} style={{ padding: '0.75rem', cursor: 'pointer', borderBottom: index < addressSuggestions.length - 1 ? '1px solid var(--border-color)' : 'none', fontSize: '14px', color: 'var(--text-primary)', backgroundColor: highlightedAddressIndex === index ? 'rgba(78, 205, 196, 0.2)' : 'var(--bg-primary)' }}>{suggestion.displayName}</div>
                    ))}
                  </div>
                )}
              </div>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            {selectedSchoolLocal ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.75rem 0.75rem 0.75rem 1rem', backgroundColor: 'var(--bg-primary)', borderRadius: '12px', border: '1px solid var(--border-color)', height: '46px', boxSizing: 'border-box' }}>
                ...
                <div style={{ flex: 1, fontSize: '14px', color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{getSchoolDisplayName(selectedSchoolLocal.name)}</div>
              ...
              <div style={{ position: 'relative' }}>
                <input ref={schoolInputRef} type="text" value={schoolQuery} onChange={(e) => setSchoolQuery(e.target.value)} onKeyDown={handleSchoolKeyDown} onFocus={() => { setIsSchoolFocused(true); if (selectedAddress && selectedAddress.coordinates) { const sorted = schools.filter(school => school.coordinates).map(school => ({ school, distance: calculateDistance(selectedAddress.coordinates, school.coordinates!) })).sort((a, b) => a.distance - b.distance).map(item => item.school); setSchoolSuggestions(sorted); setShowSchoolSuggestions(true); } else if (schoolSuggestions.length > 0) { setShowSchoolSuggestions(true); } }} onBlur={() => setTimeout(() => setIsSchoolFocused(false), 200)} placeholder="Enter your school..." style={{ width: '100%', padding: '0.75rem 0.75rem 0.75rem 1rem', border: '1px solid var(--border-color)', borderRadius: '12px', fontSize: '14px', backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)', boxSizing: 'border-box', height: '46px', outline: 'none' }} />
                {showSchoolSuggestions && schoolSuggestions.length > 0 && (
                  <div ref={schoolSuggestionsRef} style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '4px', backgroundColor: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: '12px', boxShadow: '0 2px 8px var(--shadow-hover)', maxHeight: '200px', overflowY: 'auto', zIndex: 1000 }}>
                    {schoolSuggestions.map((school, index) => {
                      ...
                      return (
                        <div key={school.id} onClick={() => handleSelectSchool(school)} onMouseEnter={() => setHighlightedSchoolIndex(index)} style={{ padding: '0.75rem', cursor: 'pointer', borderBottom: index < schoolSuggestions.length - 1 ? `1px solid ${isDarkMode ? 'rgba(255, 255, 255, 0.05)' : 'rgba(0, 0, 0, 0.05)'}` : 'none', fontSize: '14px', color: 'var(--text-primary)', backgroundColor: highlightedSchoolIndex === index ? (isDarkMode ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.05)') : 'var(--bg-primary)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={handleFindStop} disabled={!selectedAddress || !selectedSchoolLocal || isFinding} style={{ width: '100%', maxWidth: '250px', margin: '0 auto', padding: '0.875rem', fontSize: '16px', fontWeight: '600', color: (!selectedAddress || !selectedSchoolLocal || isFinding) ? textColorSecondary : (isDarkMode ? '#3A3A3A' : 'white'), backgroundColor: (!selectedAddress || !selectedSchoolLocal || isFinding) ? (isDarkMode ? 'rgba(255, 255, 255, 0.05)' : 'rgba(0, 0, 0, 0.05)') : (isDarkMode ? 'white' : '#333333'), border: (!selectedAddress || !selectedSchoolLocal || isFinding) ? `1px solid ${borderColor}` : `1px solid ${isDarkMode ? 'white' : '#333333'}`, borderRadius: '9999px', cursor: (!selectedAddress || !selectedSchoolLocal || isFinding) ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
            {isFinding ? <><div style={{ width: '60px', height: '4px', marginRight: '0.5rem' }}><ProgressBar height={4} color={isDarkMode ? '#3A3A3A' : 'white'} containerStyle={{ margin: 0 }} /></div>Finding...</> : <><MapPinIcon width={12} height={15} style={{ flexShrink: 0 }} />Find My Stop</>}
          </button>
          ...
          <div style={{ marginTop: '1.5rem', textAlign: 'center' }}>
            <Link to={selectedSchoolLocal ? `/${selectedSchoolLocal.id}/school-info` : "/schools"} onClick={() => { setRoutes([]); }} style={{ color: textColorMuted, fontSize: '14px', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              <i className="fas fa-map" style={{ fontSize: '12px' }}></i>Explore Map
            </Link>
          </div>
        </div>
      </div>
      ...
      <div style={{ width: '100%', padding: '6rem 2rem', backgroundColor: sectionBgColor, boxSizing: 'border-box', borderTop: `1px solid ${borderColor}`, borderBottom: `1px solid ${borderColor}` }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <h2 style={{ color: textColor, fontSize: '32px', fontWeight: '600', marginBottom: '4rem', textAlign: 'center' }}>Common Questions</h2>
          <div style={{ display: 'grid', gridTemplateColumns: window.innerWidth > 768 ? '1fr 1fr' : '1fr', gap: '4rem 3rem' }}>
