 ...
 */
function formatWord(word, index, words) {
 ...
 */
function formatStreetName(address) {
 ...
 * 
 * This function handles intersections by expanding each street separately
 */
function expandAddressForGeocoding(address) {
  if (!address) return address;
  
  // Remove any direction indicators in brackets at the end (e.g., "[NE]")
  const cleanAddress = address.replace(/\s*\[.*?\]\s*$/, '').trim();
  
  // Split by & or AND to handle intersections
...
export { expandAddressForGeocoding };
